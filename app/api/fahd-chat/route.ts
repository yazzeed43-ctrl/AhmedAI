...
async function getUpcomingEarnings(symbol: string, apiKey: string) {
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) return null;
    const next = items[0];
    return `âڑ ï¸ڈ ${symbol} عندها إعلان أرباح متوقع بتاريخ ${next.date} (${next.hour === "bmo" ? "قبل الافتتاح" : next.hour === "amc" ? "بعد الإغلاق" : "وقت غير محدد"}) - توقّع تقلب أعلى من المعتاد حول هذا التاريخ.`;
  } catch (e: any) {
    console.error(
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "يحسب مؤشرات فنية لسهم معين: RSI (تشبع شرائي/بيعي)طŒ MACD (زخم واتجاه)طŒ Bollinger Bands (تذبذب)طŒ ودعم/مقاومة. الدعم/المقاومة يجي من Volume Profile حقيقي (VAH/VAL/POC عبر Massive.com) لو متوفر، وإلا يرجع تلقائياً لنطاق تاريخي تقريبي (أعلى/أدنى قمة بآخر 50 شمعة) - تحقق من supportResistance.source لمعرفة أيهم رجع فعلياً. استخدمها لما يزيد يسأل عن تحليل فني، أو يسأل عن مؤشر محدد (RSIطŒ MACDطŒ دعم، مقاومة) لسهم.",
    input_schema: {
      type: "object",
      properties: {
        symbol: {
  ...
  },
  {
    name: "get_positions",
    description:
      "يجلب المراكز المفتوحة الحالية في حساب يزيد على TradierطŒ بما فيها الرمز والكمية والتكلفة. استخدمها عندما يسأل عن الصفقات أو المراكز المفتوحة أو ما يملكه حالياً.",
    input_schema: {
      type: "object",
      properties: {},
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        symbol: {
          type: "string",
          description: "رمز السهم أو ETFطŒ مثل AAPL أو SPY",
        },
      },
      required: ["symbol"],
    },
  ...
  },
  {
    ...
    description:
      "يجيب سلسلة خيارات كاملة (Calls وPuts) لسهم وتاريخ استحقاق معين، مع الأسعار وGreeks (Delta, Theta, Gamma, Vega, IV) وتقييم جودة السيولة لكل عقد (سبريد، Open InterestطŒ الحجم). âڑ ï¸ڈ بيانات Sandbox متأخرة 15 دقيقة - للتقييم والتجربة فقط، مو لقرار دخول لحظي. لازم تستخدم get_options_expirations أول لو ما عندك تاريخ استحقاق محدد.",
    input_schema: {
      type: "object",
      properties: {
        symbol: {
  ...
  },
  {
    ...
    description:
      'يحسب Volume Profile الفعلي لليوم السابق (VAHطŒ VALطŒ POC) من بيانات تداول حقيقية عبر Massive.com. âڑ ï¸ڈ لو سبق واستدعيت get_technical_indicators لنفس السهم ورجع supportResistance.source = "volume_profile"طŒ فهذي البيانات موجودة عندك مسبقاً - لا تستدعِ هذي الأداة مرة ثانية إلا لو يزيد سأل عن Volume Profile صراحة أو كان المصدر السابق "historical_range". استخدمها إلزامياً في مرحلة Zone من محرك CZT عند تحديد مناطق Previous Day VAH/VAL/POC لو ما عندك بيانات مسبقة.',
    input_schema: {
      type: "object",
      properties: {
        symbol: {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        limit: {
          type: "number",
          description: "عدد الإشارات المطلوبة، من 1 إلى 50طŒ افتراضياً 10",
          minimum: 1,
          maximum: 50,
          default: 10,
        },
...
function enrichTradierQuoteFreshness(quote: any) {
  ...
  const displayTitle =
    freshness === "live"
      ? `سعر ${safeQuote.symbol || ""} â€” Tradier (حديث جداً)`
      : freshness === "delayed"
        ? `سعر ${safeQuote.symbol || ""} â€” Tradier (قد يكون متأخراً)`
        : freshness === "stale"
          ? `سعر ${safeQuote.symbol || ""} â€” Tradier (قديم)`
          : `سعر ${safeQuote.symbol || ""} â€” Tradier (حداثة غير مؤكدة)`;
  ...
  return {
    ...
    volume_assessment: {
      allowed: false,
      reason:
        "لا توجد مقارنة Time-of-Day RVOLطŒ لذلك لا يجوز وصف الحجم بأنه منخفض أو مرتفع.",
...
// يشتغل قبل أي استدعاء لـ ClaudeطŒ عشان نوفر الوقت والتكلفة لمعظم الرسائل العادية
...
- إذا trigger.state = WAIT_TRIGGER اكتب: لا تدخل الآن.
...
6. إذا decision = WAITطŒ لا تجبر اتجاهاً واضحاً.
...
6. عند استخدام get_tradier_quoteطŒ استخدم display_title كما هو عنواناً للرد ولا تستبدله بعنوان من عندك.
7. لا تستخدم كلمة "لحظي" نهائياً إلا إذا freshness = "live". إذا كانت freshness غير liveطŒ استخدم freshness_label واذكر updated_at أو age_seconds.
...
3. Condition: تحقق من اتجاه SPY وQQQ وموضعهما من VWAP وEMA20 وEMA50 وRSIطŒ وVIX إن توفر.
4. Zone: تحقق من VAH وVAL وPOC والدعم والمقاومة وموقع السعر من VWAP.
5. Trigger: تحقق من اتجاه CALL أو PUTطŒ وإغلاق شمعة التأكيد، ومستوى الاختراق أو الكسر، والحجم النسبي إن توفر.
...
6. هذي أداة تقييم وفحص فقط، مثل باقي أدوات الخيارات — لا توصي بالدخول المباشر، اعرض الجودة والمخاطر واترك القرار النهائي ليزيد.`;
    ...
    for (let round = 0; round < maxRounds; round++) {
      ...
      // لو وصلنا آخر جولة وما زال فيه tool_useطŒ خذ أي نص متوفر كحل احتياطي
